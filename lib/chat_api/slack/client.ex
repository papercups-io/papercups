defmodule ChatApi.Slack.Client do
  ...
  alias ChatApi.{Conversations, Slack, Users}
  ...
  def send_conversation_message_alert(conversation_id, text, type: type) do
    ...
    thread = Slack.get_thread_by_conversation_id(conversation_id, channel_id)
  ...
  @spec get_slack_authorization(binary()) ::
          ...
          | Slack.SlackAuthorization.t()
  def get_slack_authorization(account_id) do
    case Slack.get_authorization_by_account(account_id) do
  ...
  @spec create_new_slack_conversation_thread(binary(), map()) ::
          {:ok, Slack.SlackConversationThread.t()} | {:error, Ecto.Changeset.t()}
